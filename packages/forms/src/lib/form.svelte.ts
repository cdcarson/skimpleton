...
export const enhanceRemoteFunctionForm = <Schema extends ZFormObject>(
  ...
  const toast = ToastService.get();
  const result = $derived(formFunction.result);
  return formFunction.enhance(async ({ submit, data: formData, form }) => {
    if (!formState.valid) {
      formState.touchAll();
      toast.error(getErrorToastMessage(formState.errors, options.errorMessage));
      return;
    }
    toast.wait(options.waitMessage || 'Please wait...');
