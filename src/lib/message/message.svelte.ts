...
import type { MessageData, MessageConfig } from './types.js';
 ...
 * MessageService.configure({ maxRecentMessages: 50 });
 * const msg = MessageService.get();
 * ```
 */
export class MessageService {
  private static _instance: MessageService | undefined;
  private static _config: MessageConfig = {
    maxRecentMessages: 20,
    successTimeout: 5000,
    errorTimeout: 8000
  };

  private _current = $state<MessageData | undefined>(undefined);
  private _mostRecent = $state<MessageData[]>([]);
  private _currentTimeout: ReturnType<typeof setTimeout> | undefined;

  private constructor() {}

  /**
   * Configure the MessageService with custom options.
   * Must be called before the first call to get() to take effect.
   */
  static configure(config: MessageConfig): void {
    MessageService._config = {
      ...MessageService._config,
      ...config
    };
  }

  static get(): MessageService {
    if (!browser) {
  ...
  get current(): MessageData | undefined {
    return this._current;
  }

  get mostRecent(): MessageData[] {
  ...
  wait(message: string = 'Please wait...'): void {
    this.setMessage({ type: 'wait', message });
  }

  setMessage(appMessage: MessageData): void {
  ...
  private _addMessage(appMessage: MessageData): void {
    if (!browser) {
      // Prevent messages from being added during SSR
      if (dev) {
        console.warn(
          'MessageService: Attempted to add message during SSR:',
          appMessage
        );
      }
      return;
    ...
    if (appMessage.type !== 'wait') {
      ...
      const maxMessages = MessageService._config.maxRecentMessages ?? 20;
    ...
    if (appMessage.type === 'success' || appMessage.type === 'error') {
      const timeoutDuration =
        appMessage.type === 'success'
          ? (MessageService._config.successTimeout ?? 5000)
          : (MessageService._config.errorTimeout ?? 8000);
