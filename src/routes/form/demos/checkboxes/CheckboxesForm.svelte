<script lang="ts">
  ...
  import { PLANETS, checkboxesFormSchema, type CheckboxesFormFormData } from './schema.ts';
  import type { ActionData } from './$types.js';
  import DemoFormData from '$demo/components/DemoFormData.svelte';

  type Props = {
    actionData: ActionData;
    savedData?: CheckboxesFormFormData;
  };
  ...
  <div class="border border-gray-200">
    ...
    <form {...form.attributes()} action="?/selectDemo" class="space-y-4 p-4">
      <fieldset class="space-y-1">
        <legend>What planets have you visited?</legend>
        <div class="grid grid-cols-2 sm:grid-cols-3">
          {#each PLANETS as opt (opt)}
            <label class="flex items-center gap-2">
              <input {...form.field('planetsVisited').checkboxAttributes(opt)} />
              {opt}
            </label>
          {/each}
        </div>

        <div id={form.field('planetsVisited').id + '-description'}>
