...
import { bigintRangeFormSchema, type BigintRangeFormData } from './schema.js';
...
const COOKIE_NAME = 'data-form-demos-bigint-range';
const COOKIE_OPTS: SerializeOptions & { path: string } = {
  path: resolve('/form/demos/bigint-range'),
  httpOnly: true,
  secure: true,
  sameSite: 'lax'
};
...
const setCookie = (event: RequestEvent, data: BigintRangeFormData) => {
