...
export const actions: Actions = {
  selectDemo: async (event: RequestEvent) => {
    ...
    // Arbitrary server error: username "admin" is reserved
    if (handler.data.username.toLowerCase() === 'admin') {
      return handler.fail({ username: 'That username is not available.' });
    }
    setDemoCookie(event, textFormSchema, handler.data);
    return handler.succeed({
      message: `Saved profile for ${handler.data.username}.`
    });
  },
