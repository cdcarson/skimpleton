<script lang="ts">
  import { MessageService, type MessageData } from 'skimpleton';
  ...
  const messageService = MessageService.get();
  ...
  const getAriaLabel = (message: MessageData): string => {
    switch (message.type) {
      case 'success':
